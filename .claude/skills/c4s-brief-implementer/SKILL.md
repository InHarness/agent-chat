...
```sh
c4s list-briefs --status pending --limit 10 --project 'agent-chat' --workspace 'default'
```
...
```sh
c4s read-brief <brief-path> --project 'agent-chat' --workspace 'default'
```
...
```bash
c4s agent "Brief nie precyzuje X — czy chodzi o A czy B?" --ct brief --brief <brief-path> --project 'agent-chat' --workspace 'default'
```
...
```bash
c4s ask "Jak dziala Y w aktualnej specce?" --project 'agent-chat' --workspace 'default'
```

Continue the brief thread with `c4s agent "..." --thread <threadId> --project 'agent-chat' --workspace 'default'` (the `threadId` is printed with the answer).
...
printf '%s\n' "$PATCH_BODY" | c4s file-patch \
  ...
  --project 'agent-chat' --workspace 'default'
```
...
```sh
c4s mark-brief-implemented <brief-path> --project 'agent-chat' --workspace 'default'
```
